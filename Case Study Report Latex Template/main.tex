...
\begin{titlepage}
    ...
    {\Large \textbf{Constrained-Random Verification of a 5-Port NoC Router using SystemVerilog}}\\[1.5cm]
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[node distance=1.6cm,>=Stealth,
        dut/.style={draw, thick, rounded corners=3pt, minimum width=4.2cm, minimum height=4.6cm, align=center},
        port/.style={draw, rounded corners=2pt, minimum width=2.2cm, minimum height=0.6cm, align=center, font=\small}]
        \node[dut] (xbar) {\textbf{Crossbar + RR Arbiter}\\ \small HEAD locks output\\ \small until TAIL transfers};
        \node[port, left=1.2cm of xbar, yshift=1.7cm] (n) {N port: 2 VC FIFOs};
        \node[port, left=1.2cm of xbar, yshift=0.85cm] (s) {S port: 2 VC FIFOs};
        \node[port, left=1.2cm of xbar] (e) {E port: 2 VC FIFOs};
        \node[port, left=1.2cm of xbar, yshift=-0.85cm] (w) {W port: 2 VC FIFOs};
        \node[port, left=1.2cm of xbar, yshift=-1.7cm] (l) {Local: 2 VC FIFOs};
        \node[port, right=1.2cm of xbar, yshift=1.7cm] (no) {to N};
        \node[port, right=1.2cm of xbar, yshift=0.85cm] (so) {to S};
        \node[port, right=1.2cm of xbar] (eo) {to E};
        \node[port, right=1.2cm of xbar, yshift=-0.85cm] (wo) {to W};
        \node[port, right=1.2cm of xbar, yshift=-1.7cm] (lo) {to Local};
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[node distance=1.7cm,>=Stealth,
        ...
        \draw[->, thick] (drv) -- (dut);
        \draw[->, thick] (dut) -- (mon);
        \draw[->, thick] (mon.south) -- ++(0,-0.35) -| (scb.east);
        \draw[->, thick] (gen) -- (cov);
        \draw[->, thick] (gen.south) -- ++(0,-0.35) -| (scb.west);
